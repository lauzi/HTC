...
if __name__ == "__main__":
    if len(sys.argv) < 2 or (int(sys.argv[1]) > 3 and int(sys.argv[1]) < 0):
        print "[Usage] python runhorse.py [author (0-3)]"
        sys.exit()
    author_name = ['村上春樹', '張愛玲', '余光中', '九把刀']
    tmp = (unicode(author_name[int(sys.argv[1])], "utf8"))
    print unicode("你選擇了","utf8") + tmp + unicode("當作主題","utf8")
    # input interface
    ...
    # process input
    authors = ['cityup', 'love0', 'lightcenter', '9knife']
    books = [5, 2, 5, 24]

    fin = codecs.open('../data/love0/2.txt', 'r', encoding='utf8')
    for book_num in range(1,books[int(sys.argv[1])]+1):
        fin = codecs.open('../data/'+authors[int(sys.argv[1])]+'/'+str(book_num)+'.txt', 'r', encoding='utf8')
        for line in fin:
            inp = clean_str(line)
            inp = inp.strip()
            time.sleep(2)
            if len(inp) > gram_num:
                ...
                print >> fout, solve(inp, gram_dict)
        fin.close()
    fin.close()
    ftmp.close()
